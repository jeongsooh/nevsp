...
class Ocpp16Consumer(WebsocketConsumer):
    ...
    async def get_specific_response(unique_id,timeout):
        print('CradRec response 1: ',response )
        """
        Return response with given unique ID or raise an asuncio. TimeoutError
        """

        wait_until = time.time() + timeout
        try :
            # Wait for response of the Call message.
            response = await asyncio.wait_for(response_quere.get(),timeout)
